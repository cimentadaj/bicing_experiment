...
This worked for my Digital Ocean droplet 512 MB and 20 GB disk with Ubuntu 20.
...
* Allow access through MySQL's port with: `sudo ufw allow 3306`. If there's any troubleshooting with logging in, this article fixed my problems: https://www.digitalocean.com/community/tutorials/how-to-allow-remote-access-to-mysql
...
Pull a docker image with Airflow:
...
```
docker run -d -p 8080:8080 bicing
```

You can even visit the DAG manager at http://ip_of_your_server:8080/admin/ where `ip_of_your_server` is the IP of your remote server.

You need to mount the dags file as well as the entire repo with all scraping scripts.

```
# the path to an renv cache on the host machine
RENV_PATHS_CACHE_HOST="/home/cimentadaj/.local/share/renv/cache/v5/R-4.0/x86_64-pc-linux-gnu"
...
docker run -d \
           -p 8080:8080 \
           -v /home/cimentadaj/bicing_experiment/:/usr/local/bicing_experiment \
           -v /home/cimentadaj/bicing_experiment/dag/:/usr/local/airflow/dags \
...
## THis is the tutorial I need to follow. It seems docker compose is what I should use to run airflow webserver, scheduler and the other.
...
The script should do something like the chunk below. Note that if you're having trouble appending the data frame to the data base you might have to do what the answer in the SO forum suggests: https://stackoverflow.com/questions/50745431/trying-to-use-r-with-mysql-the-used-command-is-not-allowed-with-this-mysql-vers?rq=1
